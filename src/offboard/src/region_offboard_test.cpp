...
void FlyState_update()
{
    ...
    switch(FlyState_1)
	{
		...
		case FLY:
			{
				if (abs(pos_drone_1.pose.position.x - setpoints_1[now_1][0]) < 1 && abs(pos_drone_1.pose.position.y - setpoints_1[now_1][1]) < 1)
                    ++now_1;
...
int main(int argc, char **argv)
{
    ...
    cal_uav_0(length/2, width, -350, 0, 100);
    cal_uav_1(length/2, width, 350, 0, 100);
